...
| --- | --- | --- |
| Bundled with the editor | publish it, then switch **Ships with the editor** on in the catalogue; the next `npm run build:definitions` carries it | everyone, offline included |
...
| --- | --- |
| a dial, gauge or meter | **`circular-gauge`** — 104 lines, the smallest complete object |
...
| drawn rather than written | **`compass`** again — its rose and needle are [imported SVG](../../scripts/blocks/definitions/art/), wired by id, with the labels left to `label-ring` |
| one object drawn several ways | **`steering-wheel`** — a drawing per type, each imported from its own SVG and carried by a `when` on a `choose` with an `equals` |
| an object with parts the reader switches on and off | **`steering-wheel`** again — a wheel and a pair of pedals, each drawn in a 200×200 art box scaled into a lane of its own, the lanes worked out from which parts are shown, the rows of a hidden part taken out of the menus by `visibleWhen`, and a `press-and-slide` area over each pedal — with a part nested inside a part, the brake switched off on its own, taking its term and its end with it and leaving the accelerator the whole of the lane |
| composed of several sub-objects | **`analogue-clock`**, **`orbit-system`** |
...
| a wave built out of what a term has been doing | **`mechanical-wave`** — a `past` binding under a `repeat`, chosen by `defines` against the object's own formula, a `swing` binding fitting the drawing to what the run has shown, and a `valueSource` that hands the model its reference oscillator when the model leaves the name free |
| an object handing the model a wave a point at a time | **`piano`** — a `valueSource` summed `over` the notes being held, stepped along a window of its own by an `{ "independent": "iteration" }` binding, so the name it writes is a plain term whose points make the wave |
| an instrument reading such a wave | **`oscilloscope`** — a `terms` list read with `termsRow`, drawn over `plot-grid`/`plot-axes`, with a `follow-pointer` cursor reading every wave where it stands and a `clickable` mark to measure the distance from. Its `element` bindings read a name the model defines over element indices element by element, and any other name over the run, a point to each iteration |
| a list of terms the reader adds to | **`oscilloscope`** again — one `repeat` over `termsCount` for the readings and one over the rows times the samples for the traces, since a repeat inside a repeat would take the outer `$index` away |
