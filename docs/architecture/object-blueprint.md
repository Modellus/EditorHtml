...
| an object handing the model a wave a point at a time | [`piano.json`](../../scripts/blocks/definitions/piano.json) — a `valueSource` summed `over` the notes being held, stepped along a window of its own by an `{ "independent": "iteration" }` binding, so the name it writes is a plain term whose points make the wave |
| an instrument reading such a wave | [`oscilloscope.json`](../../scripts/blocks/definitions/oscilloscope.json) — a `terms` list read with `termsRow`, drawn over `plot-grid`/`plot-axes`, with a `follow-pointer` cursor reading every wave where it stands and a `clickable` mark to measure the distance from. Its `element` bindings read a name the model defines over element indices element by element, and any other name over the run, a point to each iteration |
