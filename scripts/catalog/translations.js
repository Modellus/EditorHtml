export class CatalogTranslations {
  constructor(language = "en-US") {
    this.language = language in this._strings ? language : "en-US";
  }

  get(key) {
    return this._strings[this.language]?.[key] ?? this._strings["en-US"][key] ?? key;
  }

  get _strings() {
    return CatalogTranslations._strings;
  }

  static get dxMessagesLocale() {
    return "pt";
  }

  static buildDevExtremeMessages() {
    return {
      "pt": {
        "dxDataGrid-columnChooserTitle": "Escolher colunas",
        "dxDataGrid-columnChooserEmptyText": "Arraste uma coluna para aqui para a esconder",
        "dxDataGrid-groupContinuedMessage": "Continuação da página anterior",
        "dxDataGrid-groupContinuesMessage": "Continua na próxima página",
        "dxDataGrid-editingEditRow": "Editar",
        "dxDataGrid-editingSaveRowChanges": "Guardar",
        "dxDataGrid-editingCancelRowChanges": "Cancelar",
        "dxDataGrid-editingDeleteRow": "Eliminar",
        "dxDataGrid-editingUndeleteRow": "Restaurar",
        "dxDataGrid-editingConfirmDeleteMessage": "Tem a certeza que pretende eliminar este registo?",
        "dxDataGrid-editingConfirmDeleteTitle": "",
        "dxDataGrid-validationCancelChanges": "Cancelar alterações",
        "dxDataGrid-groupPanelEmptyText": "Arraste uma coluna para aqui para agrupar",
        "dxDataGrid-noDataText": "Sem dados",
        "dxDataGrid-searchPanelPlaceholder": "Pesquisar...",
        "dxDataGrid-filterRowShowAllText": "(Tudo)",
        "dxDataGrid-filterRowResetOperationText": "Repor",
        "dxDataGrid-filterRowOperationEquals": "Igual a",
        "dxDataGrid-filterRowOperationNotEquals": "Diferente de",
        "dxDataGrid-filterRowOperationLess": "Menor que",
        "dxDataGrid-filterRowOperationLessOrEquals": "Menor ou igual a",
        "dxDataGrid-filterRowOperationGreater": "Maior que",
        "dxDataGrid-filterRowOperationGreaterOrEquals": "Maior ou igual a",
        "dxDataGrid-filterRowOperationStartsWith": "Começa por",
        "dxDataGrid-filterRowOperationContains": "Contém",
        "dxDataGrid-filterRowOperationNotContains": "Não contém",
        "dxDataGrid-filterRowOperationEndsWith": "Termina em",
        "dxDataGrid-filterRowOperationBetween": "Entre",
        "dxDataGrid-filterRowOperationBetweenStartText": "Início",
        "dxDataGrid-filterRowOperationBetweenEndText": "Fim",
        "dxDataGrid-applyFilterText": "Aplicar filtro",
        "dxDataGrid-trueText": "verdadeiro",
        "dxDataGrid-falseText": "falso",
        "dxDataGrid-sortingAscendingText": "Ordem crescente",
        "dxDataGrid-sortingDescendingText": "Ordem decrescente",
        "dxDataGrid-sortingClearText": "Limpar ordem",
        "dxDataGrid-editingSaveAllChanges": "Guardar alterações",
        "dxDataGrid-editingCancelAllChanges": "Cancelar alterações",
        "dxDataGrid-editingAddRow": "Adicionar linha",
        "dxDataGrid-summaryMin": "Mín: {0}",
        "dxDataGrid-summaryMinOtherColumn": "Mín de {1} é {0}",
        "dxDataGrid-summaryMax": "Máx: {0}",
        "dxDataGrid-summaryMaxOtherColumn": "Máx de {1} é {0}",
        "dxDataGrid-summaryAvg": "Méd: {0}",
        "dxDataGrid-summaryAvgOtherColumn": "Méd de {1} é {0}",
        "dxDataGrid-summarySum": "Soma: {0}",
        "dxDataGrid-summarySumOtherColumn": "Soma de {1} é {0}",
        "dxDataGrid-summaryCount": "Contagem: {0}",
        "dxDataGrid-columnFixingFix": "Fixar",
        "dxDataGrid-columnFixingUnfix": "Soltar",
        "dxDataGrid-columnFixingLeftPosition": "À esquerda",
        "dxDataGrid-columnFixingRightPosition": "À direita",
        "dxDataGrid-exportTo": "Exportar",
        "dxDataGrid-exportAll": "Exportar tudo para Excel",
        "dxDataGrid-exportSelectedRows": "Exportar linhas seleccionadas para Excel",
        "dxDataGrid-headerFilterLabel": "Filtro",
        "dxDataGrid-headerFilterOK": "OK",
        "dxDataGrid-headerFilterCancel": "Cancelar",
        "dxDataGrid-headerFilterEmptyValue": "(vazios)",
        "dxDataGrid-headerFilterBlank": "(em branco)",
        "dxDataGrid-masterDetailExpandTooltip": "Expandir",
        "dxDataGrid-masterDetailCollapseTooltip": "Recolher",
        "dxDataGrid-rowExpanded": "Ativo",
        "dxDataGrid-rowCollapsed": "Inativo",
        "dxDataGrid-selectionSelectAllCheckboxTooltip": "Selecionar todos",
        "dxDataGrid-rowDraggingFromText": "Mover de: {0}",
        "dxDataGrid-rowDraggingToText": "Para: {0}",
        "dxPager-infoText": "Página {0} de {1} ({2} itens)",
        "dxPager-pagesCountText": "de",
        "dxPager-pageSizesAllText": "Todos",
        "dxPager-page": "Página",
        "dxPager-prevPage": "Página anterior",
        "dxPager-nextPage": "Próxima página",
        "dxPager-firstPage": "Primeira página",
        "dxPager-lastPage": "Última página",
        "dxList-pullingDownText": "Puxe para atualizar...",
        "dxList-pulledDownText": "Solte para atualizar...",
        "dxList-refreshingText": "A atualizar...",
        "dxList-pageLoadingText": "A carregar...",
        "dxList-nextButtonText": "Mais",
        "dxList-editingDeleteText": "Eliminar",
        "dxList-editingUndeleteText": "Restaurar",
        "dxList-selectAll": "Selecionar todos",
        "dxSelectBox-noDataText": "Sem dados",
        "dxSelectBox-readOnlyLabel": "Só leitura",
        "dxDropDownEditor-selectLabel": "Selecionar",
        "dxTextEditor-textEditorInputLabel": "Valor",
        "dxFileUploader-selectFile": "Selecionar ficheiro",
        "dxFileUploader-dropFile": "ou arrastar um ficheiro para aqui",
        "dxFileUploader-bytes": "bytes",
        "dxFileUploader-kb": "KB",
        "dxFileUploader-Mb": "MB",
        "dxFileUploader-Gb": "GB",
        "dxFileUploader-upload": "Enviar",
        "dxFileUploader-uploaded": "Enviado",
        "dxFileUploader-readyToUpload": "Pronto para enviar",
        "dxFileUploader-uploadAbortedMessage": "Envio cancelado",
        "dxFileUploader-uploadFailedMessage": "Envio falhou",
        "dxFileUploader-invalidFileExtension": "Tipo de ficheiro não permitido",
        "dxFileUploader-invalidMaxFileSize": "O ficheiro é demasiado grande",
        "dxFileUploader-invalidMinFileSize": "O ficheiro é demasiado pequeno",
        "dxPopup-closeLabel": "Fechar",
        "dxForm-optionalMark": "opcional",
        "dxForm-requiredMessage": "{0} é obrigatório"
      }
    };
  }
}

CatalogTranslations._strings = {
  "en-US": {
    "My Models": "My Models",
    "My Library": "My Library",
    "Discover": "Discover",
    "System": "System",
    "Favorites": "Favorites",
    "Drafts": "Drafts",
    "Trash": "Trash",
    "Published": "Published",
    "Upload": "Upload",
    "Upload Video": "Upload Video",
    "Upload Audio": "Upload Audio",
    "Upload Image": "Upload Image",
    "Upload Data": "Upload Data",
    "Add Character": "Add Character",
    "Add Object": "Add Object",
    "Create Model": "Create Model",
    "Create Notebook": "Create Notebook",
    "Search by name, author, science…": "Search by name, author, science…",
    "Filter": "Filter",
    "Clear filters": "Clear filters",
    "All": "All",
    "Kind": "Kind",
    "Board": "Board",
    "Notebook": "Notebook",
    "Recently updated": "Recently updated",
    "Recently created": "Recently created",
    "Most used": "Most used",
    "Title A–Z": "Title A–Z",
    "Grid view": "Grid view",
    "List view": "List view",
    "models": "models",
    "videos": "videos",
    "audios": "audios",
    "images": "images",
    "data sets": "data sets",
    "characters": "characters",
    "objects": "objects",
    "assets": "assets",
    "Personal": "Personal",
    "Favorite": "Favorite",
    "Library": "Library",
    "Marketplace": "Marketplace",
    "Community": "Community",
    "Samples": "Samples",
    "Assets": "Assets",
    "Audios": "Audios",
    "Images": "Images",
    "Objects": "Objects",
    "Sample": "Sample",
    "Set as sample": "Set as sample",
    "Remove from samples": "Remove from samples",
    "Education Levels": "Education Levels",
    "Sciences": "Sciences",
    "Maintenance": "Maintenance",
    "Models": "Models",
    "Notifications": "Notifications",
    "Users": "Users",
    "Create": "Create",
    "Create model": "Create model",
    "Login": "Login",
    "My Profile": "My Profile",
    "Logout": "Logout",
    "Profile": "Profile",
    "Role": "Role",
    "Country": "Country",
    "Language": "Language",
    "Save": "Save",
    "Saving…": "Saving…",
    "Profile saved.": "Profile saved.",
    "Failed to save profile.": "Failed to save profile.",
    "Teacher": "Teacher",
    "Student": "Student",
    "Select a role": "Select a role",
    "Select a country": "Select a country",
    "Select a language": "Select a language",
    "Name": "Name",
    "Email": "Email",
    "Last Login": "Last Login",
    "Created": "Created",
    "Title": "Title",
    "Description": "Description",
    "Creator": "Creator",
    "Level": "Level",
    "Science": "Science",
    "Modified": "Modified",
    "Status": "Status",
    "Message": "Message",
    "Date": "Date",
    "Fav": "Fav",
    "Color": "Color",
    "Icon": "Icon",
    "Library column": "Library",
    "Delete selected": "Delete selected",
    "Search...": "Search...",
    "No features assigned": "No features assigned",
    "Select feature flag": "Select feature flag",
    "Add": "Add",
    "Features": "Features",
    "Uncategorized": "Uncategorized",
    "No description provided.": "No description provided.",
    "New Model": "New Model",
    "Favorite action": "Favorite",
    "Unfavorite action": "Unfavorite",
    "Like action": "Like",
    "Unlike action": "Unlike",
    "Failed to update likes.": "Failed to update likes.",
    "Add to library": "Add to library",
    "In library": "In library",
    "Remove from library": "Remove from library",
    "Make public": "Make public",
    "Make private": "Make private",
    "Public": "Public",
    "Private": "Private",
    "Delete model": "Delete model",
    "Recover model": "Recover model",
    "Edit model": "Edit model",
    "Edit Model": "Edit Model",
    "More actions": "More actions",
    "Open": "Open",
    "Duplicate": "Duplicate",
    "Open in new tab": "Open in new tab",
    "Copy link": "Copy link",
    "Link copied.": "Link copied.",
    "Failed to copy link.": "Failed to copy link.",
    "Loading models…": "Loading models…",
    "Creating model…": "Creating model…",
    "Model created.": "Model created.",
    "Duplicating model…": "Duplicating model…",
    "Model duplicated.": "Model duplicated.",
    "Deleting model…": "Deleting model…",
    "Model deleted.": "Model deleted.",
    "Failed to load models.": "Failed to load models.",
    "Failed to create model.": "Failed to create model.",
    "Failed to duplicate model.": "Failed to duplicate model.",
    "Failed to delete model.": "Failed to delete model.",
    "Failed to mark favorite.": "Failed to mark favorite.",
    "Failed to update library.": "Failed to update library.",
    "Failed to update visibility.": "Failed to update visibility.",
    "Failed to update model metadata.": "Failed to update model metadata.",
    "No models found.": "No models found.",
    "Setting public…": "Setting public…",
    "Setting private…": "Setting private…",
    "Model is public.": "Model is public.",
    "Model is private.": "Model is private.",
    "Deleting selected items…": "Deleting selected items…",
    "Selected items deleted.": "Selected items deleted.",
    "Failed to delete selected items.": "Failed to delete selected items.",
    "Sign-in required to create a model.": "Sign-in required to create a model.",
    "Missing user id for model creation.": "Missing user id for model creation.",
    "Sign-in required to delete a model.": "Sign-in required to delete a model.",
    "Only the owner can delete this model.": "Only the owner can delete this model.",
    "Sign-in required to update visibility.": "Sign-in required to update visibility.",
    "Missing user id for visibility update.": "Missing user id for visibility update.",
    "Delete model?": "Delete model?",
    "This action cannot be undone.": "This action cannot be undone.",
    "Cancel": "Cancel",
    "Delete": "Delete",
    "System Templates": "System Templates",
    "Template": "Template",
    "Set as system template": "Set as system template",
    "Remove from system templates": "Remove from system templates",
    "Failed to update system template.": "Failed to update system template.",
    "Choose a starting point": "Choose a starting point",
    "Blank model": "Blank model",
    "What's New": "What's New",
    "Whats New Title field": "Title",
    "Whats New Date field": "Date",
    "Whats New Description field": "Description",
    "Whats New Image field": "Image",
    "Whats New Add Entry": "Add Entry",
    "Whats New Save Entry": "Save",
    "Whats New Entry saved.": "Entry saved.",
    "Whats New Failed to save entry.": "Failed to save entry.",
    "Whats New Failed to delete entry.": "Failed to delete entry.",
    "Overview": "Overview",
    "System overview": "System overview",
    "Overall system usage": "Overall system usage",
    "A live view of the Modellus community, content and engagement.": "A live view of the Modellus community, content and engagement.",
    "Key usage metrics": "Key usage metrics",
    "Published models": "Published models",
    "Likes": "Likes",
    "Times used": "Times used",
    "All models created": "All models created",
    "of all models": "of all models",
    "countries represented": "countries represented",
    "Across all models": "Across all models",
    "Curated examples": "Curated examples",
    "Total model usage": "Total model usage",
    "Engagement highlights": "Engagement highlights",
    "Publication rate": "Publication rate",
    "models shared publicly": "models shared publicly",
    "Average usage": "Average usage",
    "uses per model": "uses per model",
    "Average likes": "Average likes",
    "likes per published model": "likes per published model",
    "Countries": "Countries",
    "Profiles": "Profiles",
    "Where users are located": "Where users are located",
    "Community roles": "Community roles",
    "Models by subject area": "Models by subject area",
    "Models by learning stage": "Models by learning stage",
    "Not specified": "Not specified",
    "No data available": "No data available",
    "Loading system usage…": "Loading system usage…",
    "Refresh": "Refresh",
    "Dashboard unavailable": "Dashboard unavailable",
    "Try again": "Try again",
    "Growth": "Growth",
    "Average growth": "Average growth",
    "countries": "countries",
    "day": "day",
    "Users and countries per day since": "Users and countries per day since",
    "Top models": "Top models",
    "By total usage and distinct users": "By total usage and distinct users",
    "usage": "usage",
    "users": "users",
    "different user countries": "different user countries",
    "Untitled model": "Untitled model",
    "Listen to this audio": "Listen to this audio",
    "See this image": "See this image",
    "Watch this video": "Watch this video",
    "Preview this data": "Preview this data",
    "Play this animation": "Play this animation",
    "This character has no animation": "This character has no animation",
    "Failed to load the preview": "Failed to load the preview",
    "The preview shows the start of the file": "The preview shows the start of the file",
    "rows": "rows",
    "columns": "columns"
  },
  "pt-PT": {
    "My Models": "Os meus Modelos",
    "My Library": "A minha Biblioteca",
    "Discover": "Descobrir",
    "System": "Sistema",
    "Favorites": "Favoritos",
    "Drafts": "Rascunhos",
    "Trash": "Reciclagem",
    "Published": "Publicados",
    "Upload": "Carregar",
    "Upload Video": "Carregar Vídeo",
    "Upload Audio": "Carregar Áudio",
    "Upload Image": "Carregar Imagem",
    "Upload Data": "Carregar Dados",
    "Add Character": "Adicionar Personagem",
    "Add Object": "Adicionar Objeto",
    "Create Model": "Criar Modelo",
    "Create Notebook": "Criar Caderno",
    "Search by name, author, science…": "Pesquisar por nome, autor, ciência…",
    "Filter": "Filtrar",
    "Clear filters": "Limpar filtros",
    "All": "Todos",
    "Kind": "Tipo",
    "Board": "Quadro",
    "Notebook": "Caderno",
    "Recently updated": "Modificados recentemente",
    "Recently created": "Criados recentemente",
    "Most used": "Mais utilizados",
    "Title A–Z": "Título A–Z",
    "Grid view": "Vista em grelha",
    "List view": "Vista em lista",
    "models": "modelos",
    "videos": "vídeos",
    "audios": "áudios",
    "images": "imagens",
    "data sets": "conjuntos de dados",
    "characters": "personagens",
    "objects": "objetos",
    "assets": "recursos",
    "Personal": "Pessoais",
    "Favorite": "Favoritos",
    "Library": "Biblioteca",
    "Marketplace": "Mercado",
    "Community": "Comunidade",
    "Samples": "Exemplos",
    "Assets": "Recursos",
    "Audios": "Áudios",
    "Images": "Imagens",
    "Objects": "Objetos",
    "Sample": "Exemplo",
    "Set as sample": "Definir como exemplo",
    "Remove from samples": "Remover dos exemplos",
    "Education Levels": "Níveis de Ensino",
    "Sciences": "Ciências",
    "Maintenance": "Manutenção",
    "Models": "Modelos",
    "Notifications": "Notificações",
    "Users": "Utilizadores",
    "Create": "Criar",
    "Create model": "Criar modelo",
    "Login": "Iniciar sessão",
    "My Profile": "O meu Perfil",
    "Logout": "Terminar sessão",
    "Profile": "Perfil",
    "Role": "Função",
    "Country": "País",
    "Language": "Idioma",
    "Save": "Guardar",
    "Saving…": "A guardar…",
    "Profile saved.": "Perfil guardado.",
    "Failed to save profile.": "Erro ao guardar o perfil.",
    "Teacher": "Professor",
    "Student": "Estudante",
    "Select a role": "Selecione uma função",
    "Select a country": "Selecione um país",
    "Select a language": "Selecione um idioma",
    "Name": "Nome",
    "Email": "Email",
    "Last Login": "Último acesso",
    "Created": "Criado",
    "Title": "Título",
    "Description": "Descrição",
    "Creator": "Criador",
    "Level": "Nível",
    "Science": "Ciência",
    "Modified": "Modificado",
    "Status": "Estado",
    "Message": "Mensagem",
    "Date": "Data",
    "Fav": "Fav",
    "Color": "Cor",
    "Icon": "Ícone",
    "Library column": "Biblioteca",
    "Delete selected": "Eliminar selecionados",
    "Search...": "Pesquisar...",
    "No features assigned": "Sem funcionalidades atribuídas",
    "Select feature flag": "Selecione uma funcionalidade",
    "Add": "Adicionar",
    "Features": "Funcionalidades",
    "Uncategorized": "Sem categoria",
    "No description provided.": "Sem descrição.",
    "New Model": "Novo Modelo",
    "Favorite action": "Favorito",
    "Unfavorite action": "Remover favorito",
    "Like action": "Gosto",
    "Unlike action": "Remover gosto",
    "Failed to update likes.": "Erro ao atualizar gostos.",
    "Add to library": "Adicionar à biblioteca",
    "In library": "Na biblioteca",
    "Remove from library": "Remover da biblioteca",
    "Make public": "Tornar público",
    "Make private": "Tornar privado",
    "Public": "Público",
    "Private": "Privado",
    "Delete model": "Eliminar modelo",
    "Recover model": "Recuperar modelo",
    "Edit model": "Editar modelo",
    "Edit Model": "Editar modelo",
    "More actions": "Mais ações",
    "Open": "Abrir",
    "Duplicate": "Duplicar",
    "Open in new tab": "Abrir em novo separador",
    "Copy link": "Copiar ligação",
    "Link copied.": "Ligação copiada.",
    "Failed to copy link.": "Falha ao copiar a ligação.",
    "Loading models…": "A carregar modelos…",
    "Creating model…": "A criar modelo…",
    "Model created.": "Modelo criado.",
    "Duplicating model…": "A duplicar modelo…",
    "Model duplicated.": "Modelo duplicado.",
    "Deleting model…": "A eliminar modelo…",
    "Model deleted.": "Modelo eliminado.",
    "Failed to load models.": "Erro ao carregar modelos.",
    "Failed to create model.": "Erro ao criar modelo.",
    "Failed to duplicate model.": "Erro ao duplicar modelo.",
    "Failed to delete model.": "Erro ao eliminar modelo.",
    "Failed to mark favorite.": "Erro ao marcar favorito.",
    "Failed to update library.": "Erro ao atualizar biblioteca.",
    "Failed to update visibility.": "Erro ao atualizar visibilidade.",
    "Failed to update model metadata.": "Erro ao atualizar metadados.",
    "No models found.": "Nenhum modelo encontrado.",
    "Setting public…": "A tornar público…",
    "Setting private…": "A tornar privado…",
    "Model is public.": "Modelo público.",
    "Model is private.": "Modelo privado.",
    "Deleting selected items…": "A eliminar selecionados…",
    "Selected items deleted.": "Selecionados eliminados.",
    "Failed to delete selected items.": "Erro ao eliminar selecionados.",
    "Sign-in required to create a model.": "É necessário iniciar sessão para criar um modelo.",
    "Missing user id for model creation.": "ID de utilizador em falta.",
    "Sign-in required to delete a model.": "É necessário iniciar sessão para eliminar um modelo.",
    "Only the owner can delete this model.": "Apenas o proprietário pode eliminar este modelo.",
    "Sign-in required to update visibility.": "É necessário iniciar sessão para atualizar a visibilidade.",
    "Missing user id for visibility update.": "ID de utilizador em falta.",
    "Delete model?": "Eliminar modelo?",
    "This action cannot be undone.": "Esta ação não pode ser desfeita.",
    "Cancel": "Cancelar",
    "Delete": "Eliminar",
    "System Templates": "Modelos de Sistema",
    "Template": "Modelo de sistema",
    "Set as system template": "Definir como modelo de sistema",
    "Remove from system templates": "Remover dos modelos de sistema",
    "Failed to update system template.": "Erro ao atualizar modelo de sistema.",
    "Choose a starting point": "Escolher um ponto de partida",
    "Blank model": "Modelo em branco",
    "What's New": "O que há de novo",
    "Whats New Title field": "Título",
    "Whats New Date field": "Data",
    "Whats New Description field": "Descrição",
    "Whats New Image field": "Imagem",
    "Whats New Add Entry": "Adicionar entrada",
    "Whats New Save Entry": "Guardar",
    "Whats New Entry saved.": "Entrada guardada.",
    "Whats New Failed to save entry.": "Erro ao guardar entrada.",
    "Whats New Failed to delete entry.": "Erro ao eliminar entrada.",
    "Overview": "Visão geral",
    "System overview": "Visão geral do sistema",
    "Overall system usage": "Utilização global do sistema",
    "A live view of the Modellus community, content and engagement.": "Uma visão atual da comunidade, conteúdos e envolvimento no Modellus.",
    "Key usage metrics": "Principais métricas de utilização",
    "Published models": "Modelos publicados",
    "Likes": "Gostos",
    "Times used": "Utilizações",
    "All models created": "Todos os modelos criados",
    "of all models": "de todos os modelos",
    "countries represented": "países representados",
    "Across all models": "Em todos os modelos",
    "Curated examples": "Exemplos selecionados",
    "Total model usage": "Utilizações totais dos modelos",
    "Engagement highlights": "Destaques de envolvimento",
    "Publication rate": "Taxa de publicação",
    "models shared publicly": "modelos partilhados publicamente",
    "Average usage": "Utilização média",
    "uses per model": "utilizações por modelo",
    "Average likes": "Média de gostos",
    "likes per published model": "gostos por modelo publicado",
    "Countries": "Países",
    "Profiles": "Perfis",
    "Where users are located": "Localização dos utilizadores",
    "Community roles": "Perfis da comunidade",
    "Models by subject area": "Modelos por área científica",
    "Models by learning stage": "Modelos por nível de ensino",
    "Not specified": "Não especificado",
    "No data available": "Sem dados disponíveis",
    "Loading system usage…": "A carregar utilização do sistema…",
    "Refresh": "Atualizar",
    "Dashboard unavailable": "Painel indisponível",
    "Try again": "Tentar novamente",
    "Growth": "Crescimento",
    "Average growth": "Crescimento médio",
    "countries": "países",
    "day": "dia",
    "Users and countries per day since": "Utilizadores e países por dia desde",
    "Top models": "Modelos mais usados",
    "By total usage and distinct users": "Por utilizações totais e utilizadores distintos",
    "usage": "utilizações",
    "users": "utilizadores",
    "different user countries": "países diferentes de utilizadores",
    "Untitled model": "Modelo sem título",
    "Listen to this audio": "Ouvir este áudio",
    "See this image": "Ver esta imagem",
    "Watch this video": "Ver este vídeo",
    "Preview this data": "Pré-visualizar estes dados",
    "Play this animation": "Reproduzir esta animação",
    "This character has no animation": "Esta personagem não tem animação",
    "Failed to load the preview": "Não foi possível carregar a pré-visualização",
    "The preview shows the start of the file": "A pré-visualização mostra o início do ficheiro",
    "rows": "linhas",
    "columns": "colunas"
  }
};
