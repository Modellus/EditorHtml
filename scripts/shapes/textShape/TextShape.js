class TextShape extends BaseShape {
    ...
    createElement() {
        ...
        this.createEditorElements(foreignObject);
        return group;
    }

    createEditorElements(host) {
        const $wrapper = $("<div>").appendTo(host);
    ...
    getEditorToolbarWidth() {
        return this.properties.width * (this.board.svg?.getScreenCTM()?.a || 1);
    }
    ...
    positionEditorToolbar() {
        ...
        if (!anchor)
            return;
        const width = this.getEditorToolbarWidth();
