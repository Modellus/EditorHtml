...
class MediaShape extends BaseShape {

    static embedGrabBand = 12;
    ...
    showContextToolbar() {
        ...
        this._mediaSyncedSwitchInstance?.option("value", this.properties.mediaSynced !== false);
        this._videoStepsBoxInstance?.option("value", this.properties.videoStepsPerFrame);
        this._embedLinkBoxInstance?.option("value", this.properties.embedUrl ?? "");
    ...
    _updateMediaSettingsButtonIcon() {
        const instance = this._mediaSettingsDropdownElement?.dxDropDownButton("instance");
        instance?.option("icon", this._getMediaIcon());
    }

    createImageDropZoneEditor() {
        const videoUrl = this.properties.videoUrl ?? "";
        const audioUrl = this.properties.audioUrl ?? "";
        const mediaSource = videoUrl ? videoUrl : (audioUrl ? audioUrl : this.getImageSource());
    ...
    onImageControlChanged(url, mimeType) {
        ...
        if (typeof mimeType === "string" && mimeType.startsWith("video/")) {
            ...
            this._mediaAspectRatio = 0;
            this.setPropertyCommand("audioUrl", url);
        } else {
            this.properties.videoUrl = "";
            this.properties.audioUrl = "";
            this._mediaAspectRatio = 0;
    ...
    static toEmbedUrl(url) {
        ...
        if (notionEmbedUrl)
            return notionEmbedUrl;
        const youtubeId = MediaShape.getYoutubeVideoId(url);
        if (youtubeId)
    ...
    static toNotionEmbedUrl(url) {
        const match = url.match(/^https?:\/\/([\w-]+\.notion\.site)\/(?:ebd\/)?([A-Za-z0-9._~-]+)\/?(?:[?#]|$)/i);
        if (!match)
            return null;
        const pageId = match[2].match(/([0-9a-f]{32}|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/i);
        if (pageId)
            return `https://${match[1]}/ebd/${pageId[1].replace(/-/g, "")}`;
        return `https://${match[1]}/ebd/${match[2]}`;
    }
    ...
    applyEmbedLink(url) {
        if (url === "") {
            this.setPropertyCommand("embedUrl", "");
            this._updateMediaSettingsButtonIcon();
            return;
        }
        const embedUrl = MediaShape.toEmbedUrl(url);
        if (!embedUrl)
            return;
        this.properties.imageUrl = "";
        this.properties.imageBase64 = "";
        ...
        this.properties.audioUrl = "";
        this._mediaAspectRatio = 0;
        this.setPropertyCommand("embedUrl", embedUrl);
        this._updateMediaSettingsButtonIcon();
    }
    ...
    setDefaults() {
        ...
        this.properties.mediaSynced = true;
    }

    getHandles() {
        const handles = this.withEmbedHandleInset(super.getHandles());
        if (!this.properties.lockAspectRatio)
            return handles;
        return handles.map(handle => {
            if (!handle.className.includes("top-left") &&
                !handle.className.includes("top-right") &&
                !handle.className.includes("bottom-left") &&
                !handle.className.includes("bottom-right"))
                return handle;
            return Object.assign({}, handle, { getTransform: e => this.getLockedTransform(handle.className, e) });
        });
    }

    withEmbedHandleInset(handles) {
        const band = MediaShape.embedGrabBand;
        if (!this.properties.embedUrl || this.properties.width <= band * 2 || this.properties.height <= band * 2)
            return handles;
        return handles.map(handle => {
            if (handle.className === "handle move")
                return handle;
            if (handle.className === "handle rotation")
                return Object.assign({}, handle, {
                    getAttributes: () => {
                        const attributes = handle.getAttributes();
                        return Object.assign({}, attributes, { y: attributes.y + band });
                    }
                });
            return Object.assign({}, handle, { getAttributes: () => this.insetCornerHandle(handle.getAttributes(), band) });
        });
    }

    insetCornerHandle(attributes, band) {
        const position = this.getBoardPosition();
        const centerX = position.x + this.properties.width / 2;
        const centerY = position.y + this.properties.height / 2;
        const inset = band - (attributes.r ?? 0);
        return Object.assign({}, attributes, {
            cx: attributes.cx + (attributes.cx < centerX ? inset : -inset),
            cy: attributes.cy + (attributes.cy < centerY ? inset : -inset)
        });
    }
    ...
    getSelectionOutlinePrimitives() {
        ...
        return [{
            ...
            attributes: {
                x: position.x + band,
                y: position.y + band,
                width: Math.max(0, this.properties.width - band * 2),
                height: Math.max(0, this.properties.height - band * 2)
            }
        }];
    }

    updateGrabRing(position, width, height) {
        const band = MediaShape.embedGrabBand;
        this.grabRing.setAttribute("x", position.x);
        this.grabRing.setAttribute("y", position.y);
        this.grabRing.setAttribute("width", width);
        this.grabRing.setAttribute("height", height);
        this.grabRing.setAttribute("display", "inline");
        this.border.setAttribute("x", position.x + band);
        this.border.setAttribute("y", position.y + band);
        this.border.setAttribute("width", Math.max(0, width - band * 2));
        this.border.setAttribute("height", Math.max(0, height - band * 2));
    }

    getLockedTransform(className, e) {
        const x = this.properties.x;
        const y = this.properties.y;
        const w = this.properties.width;
        const h = this.properties.height;
        const ratio = this.properties.mediaAspectRatio > 0 ? this.properties.mediaAspectRatio : w / h;
        let newW, newH, newX, newY;
        if (className.includes("top-left")) {
            newW = Math.max(10, w - (e.dx + e.dy * ratio) / 2);
            newH = newW / ratio;
            newX = x + (w - newW);
            newY = y + (h - newH);
    ...
    _updateControlsOverlay() {
        ...
        if (embedUrl) {
            ...
            this._mediaOverlayDiv.style.pointerEvents = "none";
        ...
        if (videoUrl) {
            this._overlayVideoElement.controls = isSelected;
            this._overlayVideoElement.style.display = "block";
            this._overlayAudioElement.style.display = "none";
            if (this._loadedOverlayUrl !== videoUrl) {
                this._loadedOverlayUrl = videoUrl;
                this._overlayVideoElement.src = videoUrl;
            }
        } else {
            if (!this._isActive) {
                this._mediaOverlayDiv.style.display = "none";
            ...
            this._overlayVideoElement.style.display = "none";
            this._overlayAudioElement.controls = isSelected;
            this._overlayAudioElement.style.display = "block";
            if (this._loadedOverlayUrl !== audioUrl) {
                this._loadedOverlayUrl = audioUrl;
                this._overlayAudioElement.src = audioUrl;
            }
        }
    ...
    _onVideoOverlayPointerDown(event) {
        if (!this.properties.videoUrl)
            return;
        const overlayRect = this._mediaOverlayDiv.getBoundingClientRect();
        if (event.clientY > overlayRect.bottom - 54)
            return;
        event.preventDefault();
        event.stopPropagation();
        const moveHandle = this.handleElements?.find(handle => handle.classList.contains("move"));
        moveHandle?.dispatchEvent(new PointerEvent("pointerdown", event));
    }

    _onVideoOverlayClick(event) {
        if (!this.properties.videoUrl)
            return;
        const overlayRect = this._mediaOverlayDiv.getBoundingClientRect();
        if (event.clientY > overlayRect.bottom - 54)
    ...
    _positionMediaOverlay() {
        const ctm = this.board.svg.getScreenCTM();
        if (!ctm)
            return;
        const position = this.getBoardPosition();
        const { width, height } = this.properties;
        const topLeft = new DOMPoint(position.x, position.y).matrixTransform(ctm);
        const bottomRight = new DOMPoint(position.x + width, position.y + height).matrixTransform(ctm);
        ...
        if (audioUrl && !videoUrl) {
            const audioBarHeight = 40;
            this._setOverlayGeometry(topLeft.x, topLeft.y + screenHeight - audioBarHeight, screenWidth, audioBarHeight, "");
            return;
        }
        const rotationCss = rotation ? `rotate(${rotation}deg)` : "";
        this._setOverlayGeometry(topLeft.x, topLeft.y, screenWidth, screenHeight, [rotationCss, this.getFlipCssTransform()].filter(Boolean).join(" "));
    }

    _setOverlayGeometry(left, top, width, height, extraTransform) {
        const widthText = `${Math.round(width)}px`;
        const heightText = `${Math.round(height)}px`;
        if (this._overlayWidthText !== widthText) {
            this._overlayWidthText = widthText;
            this._mediaOverlayDiv.style.width = widthText;
        }
        if (this._overlayHeightText !== heightText) {
            this._overlayHeightText = heightText;
            this._mediaOverlayDiv.style.height = heightText;
        }
        this._mediaOverlayDiv.style.transform = `translate(${left}px, ${top}px) ${extraTransform}`;
    ...
    createElement() {
        ...
        this.image.addEventListener("load", () => {
            if (this.properties.mediaAspectRatio)
                return;
            const src = this.image.getAttribute("href");
            if (!src)
                return;
            const img = new Image();
            img.onload = () => {
                if (img.naturalWidth && img.naturalHeight) {
                    this.properties.mediaAspectRatio = img.naturalWidth / img.naturalHeight;
        ...
        this.videoForeignObject = this.board.createSvgElement("foreignObject");
        this.canvas = document.createElement("canvas");
        this.canvas.style.cssText = "display:block;width:100%;height:100%;";
        this.videoForeignObject.appendChild(this.canvas);
        this.videoElement = document.createElement("video");
        this.videoElement.muted = true;
        this.videoElement.preload = "auto";
        this.videoElement.style.cssText = "position:fixed;left:-9999px;top:-9999px;width:1px;height:1px;pointer-events:none;";
        document.body.appendChild(this.videoElement);
        this._loadedVideoUrl = null;
        this._blobUrl = null;
        this._videoReady = false;
        this._isSeeking = false;
        this._pendingFrameIndex = -1;
        this._lastRenderedFrameIndex = -1;
        this.videoElement.addEventListener("seeked", () => {
            this._isSeeking = false;
            this.canvas.getContext("2d").drawImage(this.videoElement, 0, 0, this.canvas.width, this.canvas.height);
            this._lastRenderedFrameIndex = this._pendingFrameIndex;
            this.board.markDirty(this);
        });
        element.appendChild(this.videoForeignObject);
        this._mediaOverlayDiv = document.createElement("div");
        this._mediaOverlayDiv.style.cssText = "position:fixed;left:0;top:0;display:none;z-index:100;overflow:hidden;will-change:transform;";
        this._overlayVideoElement = document.createElement("video");
        ...
        this._overlayFrameElement.style.cssText = `position:absolute;left:${MediaShape.embedGrabBand}px;top:${MediaShape.embedGrabBand}px;width:calc(100% - ${MediaShape.embedGrabBand * 2}px);height:calc(100% - ${MediaShape.embedGrabBand * 2}px);display:none;border:0;pointer-events:auto;`;
        ...
        document.body.appendChild(this._mediaOverlayDiv);
        this._loadedOverlayUrl = null;
        this._audioElement = document.createElement("audio");
        this._audioElement.preload = "auto";
        this._audioElement.style.cssText = "position:fixed;left:-9999px;top:-9999px;width:1px;height:1px;pointer-events:none;";
        document.body.appendChild(this._audioElement);
        this._loadedAudioUrl = null;
        this._audioBlobUrl = null;
        this._audioReady = false;
        this._audioIsSeeking = false;
        this._audioPendingFrameIndex = -1;
        this._audioLastRenderedFrameIndex = -1;
        this._audioElement.addEventListener("seeked", () => {
            this._audioIsSeeking = false;
            this._audioLastRenderedFrameIndex = this._audioPendingFrameIndex;
            this.board.markDirty(this);
        });
        this._isActive = false;
        this.grabRing = this.board.createSvgElement("rect");
        this.grabRing.setAttribute("fill", "transparent");
        this.grabRing.setAttribute("display", "none");
        element.appendChild(this.grabRing);
    ...
    update() {
        ...
        const audioUrl = this.properties.audioUrl ?? "";
        if (videoUrl)
            this.imageDropZoneControl.setVideoSource(videoUrl);
        else if (audioUrl)
            this.imageDropZoneControl.setAudioSource(audioUrl);
        else
    ...
    tick() {
        super.tick();
        if (this.properties.videoUrl && this.properties.mediaSynced !== false)
            this.board.markDirty(this);
        if (this.properties.audioUrl && this.properties.mediaSynced !== false)
            this.board.markDirty(this);
    }

    draw() {
        super.draw();
        if (this.properties.lockAspectRatio && this.properties.mediaAspectRatio > 0) {
            const correctHeight = this.properties.width / this.properties.mediaAspectRatio;
            if (Math.abs(correctHeight - this.properties.height) > 0.01) {
                this.properties.height = correctHeight;
                this.updateHandles();
            }
        }
        const position = this.getBoardPosition();
        ...
        this.videoForeignObject.setAttribute("x", position.x);
        this.videoForeignObject.setAttribute("y", position.y);
        this.videoForeignObject.setAttribute("width", width);
        this.videoForeignObject.setAttribute("height", height);
        ...
        if (embedUrl) {
            ...
            this.updateGrabRing(position, width, height);
            return;
        }
        this.grabRing.setAttribute("display", "none");
        if (videoUrl) {
            if (isNonSynced) {
                ...
                this._updateControlsOverlay();
                if (this._loadedVideoUrl !== videoUrl) {
                    ...
                    this._pendingFrameIndex = -1;
                    this._lastRenderedFrameIndex = -1;
                    this._fetchVideoBlob(videoUrl);
                }
                if (!this._videoReady) {
                    this.image.setAttribute("href", this.getImageSource());
                    this.image.setAttribute("display", "inline");
                    this.videoForeignObject.setAttribute("display", "none");
                    return;
                }
                this.image.setAttribute("display", "none");
                this.videoForeignObject.setAttribute("display", "inline");
                if (!this.videoElement.duration)
                    return;
                const stepsPerFrame = Math.max(1, this.properties.videoStepsPerFrame ?? 1);
                const iteration = this.board.calculator.getIteration();
                const frameIndex = Math.floor(iteration / stepsPerFrame);
                if (!this._isSeeking && this._lastRenderedFrameIndex !== frameIndex) {
                    this._isSeeking = true;
                    this._pendingFrameIndex = frameIndex;
                    this.videoElement.currentTime = (frameIndex / 30) % this.videoElement.duration;
                }
            }
        } else if (audioUrl) {
            const imageSource = this.getImageSource();
            if (this._loadedImageUrl !== imageSource) {
                this._loadedImageUrl = imageSource;
                this.properties.mediaAspectRatio = 0;
            }
            ...
            if (isNonSynced) {
                ...
                this._updateControlsOverlay();
                if (this._loadedAudioUrl !== audioUrl) {
                    this._loadedAudioUrl = audioUrl;
                    this._audioReady = false;
                    this._audioIsSeeking = false;
                    this._audioPendingFrameIndex = -1;
                    this._audioLastRenderedFrameIndex = -1;
                    this._fetchAudioBlob(audioUrl);
                }
                if (!this._audioReady || !this._audioElement.duration)
                    return;
                const stepsPerFrame = Math.max(1, this.properties.videoStepsPerFrame ?? 1);
                const iteration = this.board.calculator.getIteration();
                const frameIndex = Math.floor(iteration / stepsPerFrame);
                if (!this._audioIsSeeking && this._audioLastRenderedFrameIndex !== frameIndex) {
                    this._audioIsSeeking = true;
                    this._audioPendingFrameIndex = frameIndex;
                    this._audioElement.currentTime = (frameIndex / 30) % this._audioElement.duration;
                }
            }
        } else {
            this._updateControlsOverlay();
    ...
    async _fetchVideoBlob(videoUrl) {
        ...
        this.videoElement.src = this._blobUrl;
        this.videoElement.addEventListener("canplay", () => {
            this.canvas.width = this.videoElement.videoWidth || Math.round(this.properties.width);
            this.canvas.height = this.videoElement.videoHeight || Math.round(this.properties.height);
            if (this.videoElement.videoWidth && this.videoElement.videoHeight) {
                this.properties.mediaAspectRatio = this.videoElement.videoWidth / this.videoElement.videoHeight;
            }
    ...
    async _fetchAudioBlob(audioUrl) {
        ...
        this._audioElement.src = this._audioBlobUrl;
