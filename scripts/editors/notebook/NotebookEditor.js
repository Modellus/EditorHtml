...
class NotebookEditor extends Workspace {
    constructor() {
        super(null);
        ...
        this.setWorkspaceSurfaceAdapter({
            refresh: () => this.wall.refresh(),
            forceRefresh: () => this.wall.refresh()
        });
    ...
    _createBlockList() {
        const container = this.wall.getBlockHost();
        $(container).dxList({
    ...
    _renderBlockContent(block) {
        return BlocksRegistry.renderContentHtml(this, block);
    }
