...
test.describe('seeding the bundled objects', () => {
    test('a dry run plans every object and writes nothing', async ({ page }) => {
        ...
        expect(seeding.results.map(result => result.type)).toEqual([
            'calculator', 'circular-gauge', 'clock', 'compass', 'mechanical-wave', 'mouse-tracker', 'orbit-system', 'oscilloscope', 'piano', 'protractor', 'rotating-vector', 'ruler', 'speedometer', 'steering-wheel', 'thermometer'
        ]);
    ...
    test('writing creates each object with the definition and a drawn screenshot', async ({ page }) => {
        ...
        expect(state.created).toHaveLength(15);
        expect(seeding.results.map(result => result.id)).toEqual(['id-1', 'id-2', 'id-3', 'id-4', 'id-5', 'id-6', 'id-7', 'id-8', 'id-9', 'id-10', 'id-11', 'id-12', 'id-13', 'id-14', 'id-15']);
    ...
    test('seeding again leaves what is already there alone', async ({ page }) => {
        ...
        expect(state.created).toHaveLength(14);
    ...
    test('an update the catalogue refuses reports the reason it gave', async ({ page }) => {
        ...
        expect(seeding.results.filter(result => result.action === 'create')).toHaveLength(14);
    });
    ...
    test('a catalogue that cannot be listed stops a write but not a dry run', async ({ page }) => {
        ...
        expect(dryRun.results).toHaveLength(15);
